...
OcDbDatabaseImpl::OcDbDatabaseImpl(void)
...
OcApp::ErrorStatus OcDbDatabaseImpl::Open(const string_t & filename)
{
    OcBsStreamIn in;
    in.Open(filename);
    if(!in) {
    ...
    if(dwgHdr.IsR13c3OrHigher()) {
        ...
        OcDfDatabaseHeaderVars hdrVars(m_pHeaderImpl);
        ar >> hdrVars;
        if(ar.Error() != OcApp::eOk) {
...
#define DEFINE_GETTER_SETTER_BY_VAL(TYPE, FUNCTION) \
    TYPE OcDbDatabaseImpl::FUNCTION(void) const { return m_pHeaderImpl->FUNCTION(); } \
    void OcDbDatabaseImpl::FUNCTION(TYPE val) { m_pHeaderImpl->FUNCTION(val); };

#define DEFINE_GETTER_SETTER_BY_REF(TYPE, FUNCTION) \
    const TYPE & OcDbDatabaseImpl::FUNCTION(void) const { return m_pHeaderImpl->FUNCTION(); } \
    void OcDbDatabaseImpl::FUNCTION(const TYPE & val) { m_pHeaderImpl->FUNCTION(val); };
